...
class Opalmembership_Emails {
	...
	public static function init() {
		...
		add_action( 'opalmembership_update_payment_status_to_processing', [ __CLASS__, 'new_purchased_email' ], 10, 1 );
		add_action( 'opalmembership_update_payment_status_to_completed', [ __CLASS__, 'completed_email' ], 10, 1 );
		add_action( 'opalmembership_update_payment_status_to_cancelled', [ __CLASS__, 'cancelled_email' ], 10, 1 );
		add_action( 'opalmembership_update_payment_status_to_refunded', [ __CLASS__, 'refunded_email' ], 10, 1 );
		...
		add_action( 'opalmembership_process_user_package_expired', [ __CLASS__, 'send_expired_email' ], 10, 1 );
	}
	...
	public static function completed_email( $payment_id ) {
		...
		self::send( $args['user_email'], $subject, $body );
	}
