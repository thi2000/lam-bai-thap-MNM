...
if (class_exists('WooCommerce')):
    class Wpopal_Widget_Layered_Nav extends WC_Widget {

        /**
         * Constructor.
         */
        public function __construct() {
            $this->widget_cssclass    = 'woocommerce wpopal_widget_layered_nav woocommerce-widget-layered-nav';
            $this->widget_description = __( 'Display a list of attributes to filter products in your store.(OSF custom)', 'wpopal' );
            $this->widget_id          = 'wpopal_woocommerce_layered_nav';
            $this->widget_name        = __( 'WpOpal Filter Products by Attribute', 'wpopal' );
            parent::__construct();
        }
        ...
        public function init_settings() {
            ...
            $this->settings = array(
                'title' => array(
                    'type'  => 'text',
                    'std'   => __( 'Filter by', 'wpopal' ),
                    'label' => __( 'Title', 'wpopal' ),
                ),
                'attribute' => array(
                    'type'    => 'select',
                    'std'     => '',
                    'label'   => __( 'Attribute', 'wpopal' ),
                    'options' => $attribute_array,
                ),
                'display_type' => array(
                    'type'    => 'select',
                    'std'     => 'list',
                    'label'   => __( 'Display type', 'wpopal' ),
                    'options' => array(
                        'list'     => __( 'List', 'wpopal' ),
                        'dropdown' => __( 'Dropdown', 'wpopal' ),
                    ),
                ),
                'style' => array(
                    'type'    => 'select',
                    'std'     => 'one_column',
                    'label'   => __( 'Style', 'wpopal' ),
                    'options' => array(
                        'one_column'          => __( '1 Column', 'wpopal' ),
                        'two_columns'   => __( '2 Columns', 'wpopal' ),
                        'inline'        => __( 'Inline', 'wpopal' ),
                    ),
                ),
                'query_type' => array(
                    'type'    => 'select',
                    'std'     => 'and',
                    'label'   => __( 'Query type', 'wpopal' ),
                    'options' => array(
                        'and' => __( 'AND', 'wpopal' ),
                        'or'  => __( 'OR', 'wpopal' ),
                    ),
        ...
        protected function layered_nav_list( $terms, $taxonomy, $query_type, $display_type , $style ) {
            // List display.
            echo '<ul class="woocommerce-widget-layered-nav-'. $display_type .' '. $style .' taxonomy-'.$taxonomy.'">';
