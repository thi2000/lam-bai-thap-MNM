...
class OSF_Elementor_Imagehotspots_Widget extends Elementor\Widget_Base {
	...
	protected function _register_controls() {
		...
		$repeater->add_control( 'image_hotspots_product_detail',
			[
				'label'       => esc_html__( 'Product Detail', 'strollik-core' ),
							...
							if ( $item['image_hotspots_content'] == 'elementor_templates' ) {
								...
								if( $product ) {
									...
									if( $item['image_hotspots_product_detail'] == 'yes' ){
										...
										echo '<div class="star-rating" title="'.sprintf(esc_attr__( 'Rated %s out of 5', 'wpopalbootstrap' ), $average).'"><span style="width:'.( ( $average / 5 ) * 100 ) . '%"><strong class="rating">'.$average.'</strong> '.esc_html__( 'out of 5', 'wpopalbootstrap' ).'</span></div>'; 
